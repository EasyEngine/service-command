...
function init_global_container( $service, $container = '' ) {
	...
	if ( 'running' !== \EE_DOCKER::container_status( $container ) ) {
		...
		\EE_DOCKER::boot_container( $container, docker_compose_with_custom() . ' up -d ' . $service );
		return true;
	} else {
		return false;
	}
...
function set_nginx_proxy_version_conf() {
	...
	$version_success = EE::exec( sprintf( docker_compose_with_custom() . ' exec global-nginx-proxy bash -c \'echo "%s" > %s\'', $version_line, $version_file ), false, false, [
		$version_file,
		$version_line,
	] );
	if ( $version_success ) {
		EE::exec( docker_compose_with_custom() . ' exec global-nginx-proxy bash -c "nginx -t && nginx -s reload"' );
	}
}
